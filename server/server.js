...
const APP_UPDATE_MANIFESTS = {
  ...
  // No `desktop` entry: this fork is VtM Conquest, so the desktop track must
  // never surface upstream Open Historia artifacts. With no manifest the
  // /api/app-update endpoint returns {} for track=desktop and the in-app
  // update banner (which requires current/buildId/download) stays silent.
...
httpServer.on("error", (error) => {
  if (error?.code === "EADDRINUSE") {
    console.error(`Port ${PORT} is already in use — VtM Conquest is probably already running.`);
