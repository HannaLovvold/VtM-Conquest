...
const getScenarioManifest = () => {
  ...
  if (manifest && Array.isArray(manifest.order)) {
    return {
      // The built-in Modern Day scenario stays on disk as internal fallback
      // data (clean-seed resets, map borrowing) but is never shown in the
      // deck — filter it out of any manifest that still lists it.
      order: manifest.order.filter((id) => id !== DEFAULT_SCENARIO_ID),
      selectedScenarioId:
...
const saveScenarioManifest = (manifest) => {
  writeJsonFile(SCENARIO_MANIFEST_PATH, {
    ...
    order: Array.from(new Set(manifest.order ?? [])).filter((id) => id !== DEFAULT_SCENARIO_ID),
...
const ensureDefaultScenario = () => {
  ...
  // VtM Conquest hides the built-in Modern Day scenario from the deck. Its
  // data stays on disk purely as internal fallback (clean-seed resets, map
  // borrowing for scenarios without their own geometry), so keep that data
  // coherent when present, but never register or select it.
  ...
  // Migrate older manifests that still list the hidden scenario: rewrite the
  // file without it. (Selection is re-healed on the read path — the catalog
  // falls back to the first existing scenario when the saved one is missing.)
  const raw = readJsonFile(SCENARIO_MANIFEST_PATH, null);
  if (raw && Array.isArray(raw.order) && raw.order.includes(DEFAULT_SCENARIO_ID)) {
    const manifest = getScenarioManifest(); // default-filtered view
    if (!manifest.selectedScenarioId) {
      const others = fs.existsSync(SCENARIOS_DIR)
        ? fs.readdirSync(SCENARIOS_DIR, { withFileTypes: true })
          .filter((entry) => entry.isDirectory() && entry.name !== DEFAULT_SCENARIO_ID)
          .map((entry) => entry.name)
        : [];
      manifest.selectedScenarioId = others[0] ?? "";
    }
...
const buildScenarioCatalog = () => {
  ...
  // The hidden built-in scenario never appears in the deck, even though its
  // directory is still on disk as fallback data.
  const orderedScenarioIds = resolveOrderedIds(manifest.order, SCENARIOS_DIR, DEFAULT_SCENARIO_ID)
  .filter((id) => id !== DEFAULT_SCENARIO_ID);
...
const setSelectedScenario = (scenarioId) => {
  ...
  manifest.order = resolveOrderedIds(manifest.order, SCENARIOS_DIR, DEFAULT_SCENARIO_ID).filter(
    (entry) => entry !== scenarioId && entry !== DEFAULT_SCENARIO_ID,
  );
