...
// VtM FORK ISOLATION: without this, this fork would share Electron's userData
// directory with the stock Open Historia install (upstream's appId), so the
// player's regular scenario library would load here and the bundled Chicago
// scenario would never appear. Point this app at its own data root instead.
app.setPath("userData", path.join(app.getPath("appData"), "vtm-conquest"));

// Ship the fork's scenario into the (fresh) data root on first run. The server
// seeds only the built-in Modern Day default itself, so without this the
// Scenario Deck would never show Chicago by Night. resolveOrderedIds() appends
// any scenario directory it doesn't know, so no manifest edit is needed.
// Copy file-by-file: fs.cpSync cannot copy OUT of app.asar (it creates the
// target dir then copies nothing), but readFileSync/writeFileSync go through
// Electron's asar-aware fs and work fine.
const BUNDLED_SCENARIOS = path.join(__dirname, "..", "server", "data", "scenarios");
...
const seedBundledScenarios = () => {
  let entries = [];
  try {
    entries = fs.readdirSync(BUNDLED_SCENARIOS, { withFileTypes: true });
  } catch {
    return; // dev without the folder, or a read failure — nothing to seed
  }
  for (const entry of entries) {
    if (!entry.isDirectory() || entry.name === "default") continue; // default is server-seeded
    const to = path.join(DATA_DIR, "scenarios", entry.name);
    try {
      // An EMPTY leftover target (e.g. from the cpSync quirk above) is not a
      // successful seed — replace it rather than skipping.
      const seeded = fs.existsSync(to) && fs.readdirSync(to).length > 0;
      if (!seeded) {
        copyTree(path.join(BUNDLED_SCENARIOS, entry.name), to);
      }
    } catch {
      /* a failed seed must not block boot — the folder just won't be listed */
    }
  }
};
...
const startServer = async () => {
  ...
  if (port !== requested) {
    console.log(`Port ${requested} is in use — starting VtM Conquest on ${port} instead.`);
  }
