...
export const WORLD_DEFAULTS = {
  ...
  // Global VtM tracks (0-100), moved by the AI each turn via worldStateOps and
  // fed back into prompts like internationalReputation. Masquerade Integrity:
  // how intact the veil is (100 = perfect cover). Inquisition Heat: how close
  // the Second Inquisition is to burning the city (starting value is the seed;
  // the scenario's world.json overrides it).
  inquisitionHeat: 20,
  // How sound the Masquerade stands this night, 0-100 (100 = perfect cover).
  masqueradeIntegrity: 100,
...
// VtM retheme — the six Kindred force types (mechanical roles unchanged):
// coterie (baseline Kindred cell), hounds (war ghouls/beasts, heavy),
// proxies (deniable mortal assets, fast/long reach), retainers (feeding &
// logistics networks, weak), sorcery (blood-sorcery covens, long-range strike),
// havenguard (haven & court defenders, defensive).
export const UNIT_TYPES = ["coterie", "hounds", "proxies", "retainers", "sorcery", "havenguard"];
...
export const normalizeUnitEntry = (entry, index = 0) => {
  ...
  return {
    ...
    type: UNIT_TYPE_SET.has(type) ? type : "coterie",
    ownerCode,
...
// One AI-authored movement of the global VtM tracks (Masquerade Integrity /
// Inquisition Heat). Values are DELTAS — how far the event moved the track,
// negative to lower Masquerade / raise Heat — because several events in one
// jump each move them a little and absolutes would clobber each other.
const normalizeWorldStateOp = (entry) => {
  if (!entry || typeof entry !== "object") {
    return null;
  }

  const masquerade = Number(entry.masqueradeIntegrity);
  const heat = Number(entry.inquisitionHeat);
  const hasMasquerade = Number.isFinite(masquerade) && masquerade !== 0;
  const hasHeat = Number.isFinite(heat) && heat !== 0;
  if (!hasMasquerade && !hasHeat) {
    return null;
  }

  return {
    ...(hasMasquerade ? { masqueradeIntegrity: masquerade } : {}),
    ...(hasHeat ? { inquisitionHeat: heat } : {}),
...
const normalizeEventImpacts = (value) => {
  ...
  return {
    ...
    worldStateOps: normalizeArray(value.worldStateOps).map(normalizeWorldStateOp).filter(Boolean),
...
export const normalizeWorldState = (world) => {
  ...
  // Global VtM tracks: clamp to 0-100 like reputation above. Explicit (not via
  // the ...WORLD_DEFAULTS spread) so a NaN/garbage write can never poison them.
  const masqueradeIntegrity = Number.isFinite(Number(nextWorld.masqueradeIntegrity))
    ? Math.max(0, Math.min(100, Math.round(Number(nextWorld.masqueradeIntegrity))))
    : WORLD_DEFAULTS.masqueradeIntegrity;
  const inquisitionHeat = Number.isFinite(Number(nextWorld.inquisitionHeat))
    ? Math.max(0, Math.min(100, Math.round(Number(nextWorld.inquisitionHeat))))
    : WORLD_DEFAULTS.inquisitionHeat;
...
export const applyEventImpactsToWorld = ({ colors = {}, events = [], world }) => {
  ...
  for (const event of normalizeEvents(events)) {
    ...
    // Global VtM tracks: apply the AI's movements as clamped deltas. The staged
    // event reveal replays this same function, so the HUD chips animate with it.
    for (const op of event.impacts.worldStateOps) {
      if (Number.isFinite(op.masqueradeIntegrity)) {
        nextWorld.masqueradeIntegrity = Math.max(0, Math.min(100,
          Math.round(nextWorld.masqueradeIntegrity + op.masqueradeIntegrity)));
      }
      if (Number.isFinite(op.inquisitionHeat)) {
        nextWorld.inquisitionHeat = Math.max(0, Math.min(100,
          Math.round(nextWorld.inquisitionHeat + op.inquisitionHeat)));
      }
    }
