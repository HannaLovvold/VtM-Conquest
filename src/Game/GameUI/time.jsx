...
import { TEXT, TEXT_BRIGHT, FONT_UI } from "./theme";
...
const ensureTimelineStyles = () => {
    ...
    .timeline-markdown blockquote {
        border-left: 2px solid rgba(138, 3, 3, 0.55);
...
const SpinnerRing = ({ size = 14, tone = "rgba(232,221,200,0.88)" }) => {
    useEffect(() => {
    ...
    return (
        ...
        >
        <circle cx="12" cy="12" r="8" stroke="rgba(232,221,200,0.2)" strokeWidth="2.2" />
...
const panelSurface = {
    backgroundColor: "rgba(22, 15, 13, 0.95)",
    backdropFilter: "blur(8px)",
    border: "1px solid rgba(232,221,200,0.1)",
    borderRadius: "16px",
    boxShadow: "-4px 0 24px rgba(0,0,0,0.4), inset 0 1px 0 rgba(232,221,200,0.06)",
    color: TEXT,
    fontFamily: FONT_UI,
    overflow: "hidden",
    position: "fixed",
...
const widgetSurface = {
    ...
    fontFamily: FONT_UI,
    gap: "0.25rem",
    // "auto" + minHeight so the VtM track chips (Masquerade/Heat) can add a
    // second row under the date without clipping.
    height: "auto",
    minHeight: "3.5rem",
...
// Compact HUD chips for the global VtM tracks (Masquerade Integrity /
// Inquisition Heat). `badHigh` tracks (Heat) shift red as the value RISES;
// `badLow` tracks (Masquerade) shift red as it FALLS.
const trackChipStyle = (value, { badHigh = false } = {}) => {
    const severity = badHigh
        ? value >= 70 ? 0 : value >= 40 ? 1 : 2
        : value <= 30 ? 0 : value < 70 ? 1 : 2;
    const tones = [
        { color: "rgba(232,221,200,0.84)", border: "rgba(232,221,200,0.28)", background: "rgba(232,221,200,0.10)" },
        { color: "#fcd9a8", border: "rgba(245,158,11,0.5)", background: "rgba(245,158,11,0.12)" },
        { color: "#fecaca", border: "rgba(248,113,113,0.55)", background: "rgba(220,38,38,0.16)" },
    ];
    return tones[severity];
};

const TrackChip = ({ value, label, glyph, badHigh = false }) => {
    const tone = trackChipStyle(value, { badHigh });
    return (
        <span
...
const MetricPill = ({ children, icon = null, tone = "default" }) => {
    const toneMap = {
        ...
        accent: {
            ...
            border: "1px solid rgba(138, 3, 3, 0.22)",
            color: TEXT_BRIGHT,
        },
        violet: {
            background: "rgba(138, 3, 3, 0.12)",
...
const EventCard = ({ event, footer = null, lookups }) => {
    ...
    return (
        <div
        style={{
            background: "linear-gradient(180deg, rgba(232,221,200,0.055), rgba(232,221,200,0.03))",
            border: "1px solid rgba(232,221,200,0.08)",
            borderRadius: "16px",
            boxShadow: "inset 0 1px 0 rgba(232,221,200,0.03)",
            overflow: "hidden",
        }}
        >
        <div
        style={{
            alignItems: "center",
            background: "rgba(232,221,200,0.02)",
            borderBottom: "1px solid rgba(232,221,200,0.05)",
            display: "flex",
            gap: "0.45rem",
        ...
        <div style={{ color: "rgba(232,221,200,0.94)", fontSize: "0.82rem", fontWeight: 800, letterSpacing: "0.06em", textTransform: "uppercase" }}>
        {event.title}
        </div>

        {event.description && (
            <div className="timeline-markdown" style={{ color: "rgba(232,221,200,0.82)", fontSize: "0.77rem", lineHeight: "1.58" }}>
...
const EmptyPanelState = ({ text }) => (
    ...
    style={{
        ...
        background: "rgba(232,221,200,0.03)",
                                       border: "1px dashed rgba(232,221,200,0.1)",
                                       borderRadius: "16px",
                                       color: "rgba(232,221,200,0.48)",
                                       display: "flex",
                                       fontSize: "0.78rem",
                                       fontStyle: "italic",
                                       justifyContent: "center",
...
const PanelChrome = ({
    ...
    return (
        ...
        <div
        style={{
            borderBottom: hasHeaderText ? "1px solid rgba(232,221,200,0.07)" : "none",
            flexShrink: 0,
        ...
        {hasHeaderText && (
            ...
            {eyebrow && (
                <div style={{ color: "rgba(232,221,200,0.75)", fontSize: "0.64rem", fontWeight: 700, letterSpacing: "0.14em", marginBottom: "0.12rem", textTransform: "uppercase" }}>
                {eyebrow}
                </div>
            )}
            {title && (
                <div style={{ color: "rgba(232,221,200,0.96)", fontSize: "1rem", fontWeight: 700 }}>
                {title}
                </div>
            )}
            {subtitle && (
                <div style={{ color: "rgba(232,221,200,0.45)", fontSize: "0.75rem", lineHeight: "1.45", marginTop: "0.12rem" }}>
                {subtitle}
                </div>
            )}
            </div>
        ...
        onMouseEnter={(event) => {
            event.currentTarget.style.background = "rgba(232,221,200,0.08)";
            event.currentTarget.style.color = TEXT_BRIGHT;
        }}
        onMouseLeave={(event) => {
            event.currentTarget.style.background = "none";
            event.currentTarget.style.color = "rgba(232,221,200,0.5)";
        }}
...
const JumpNode = ({ isLoading, opt, onJump }) => {
    ...
    return (
        ...
        style={{
            background: hovered ? "rgba(138, 3, 3, 0.35)" : "rgba(138, 3, 3, 0.15)",
            border: hovered ? "1px solid rgba(138, 3, 3, 0.7)" : "1px solid rgba(138, 3, 3, 0.35)",
            borderRadius: "10px",
            color: TEXT,
            cursor: "pointer",
        ...
        <div style={{ color: "rgba(232,221,200,0.7)", fontSize: "0.7rem" }}>
        {opt.label}
        </div>
        </button>
    );
...
const TimelineSkipPanel = ({
    ...
    return (
        ...
        {canUndo && (
            ...
            <div style={{ background: "rgba(138, 3, 3, 0.4)", height: "1.25rem", width: "2px" }} />
            </>
        )}
        <div
        style={{
            background: "rgba(138, 3, 3, 0.2)",
            border: "2px solid rgba(138, 3, 3, 0.8)",
            borderRadius: "999px",
            color: "rgba(232,221,200,0.95)",
            fontSize: "0.7rem",
            fontWeight: 700,
        ...
        style={{
            ...
            border: "1px solid rgba(138, 3, 3, 0.6)",
            borderRadius: "8px",
            color: TEXT,
...
const DateWidget = ({
    ...
    return (
        ...
        style={{
            ...buttonStyle,
            color: openPanel === "history" ? TEXT_BRIGHT : buttonStyle.color,
        }}
        ...
        {playerCountry ? (
            ...
            <span style={{ color: "rgba(232,221,200,0.94)", flexShrink: 0, fontSize: isMobile ? "0.82rem" : "0.95rem", letterSpacing: "0.02em", whiteSpace: "nowrap" }}>
            {displayDate}
            </span>
            </div>
        ) : (
            <div style={{ color: "rgba(232,221,200,0.94)", fontSize: "0.95rem", letterSpacing: "0.02em" }}>
            {displayDate}
            </div>
        )}
        {worldState && Number.isFinite(Number(worldState.masqueradeIntegrity)) && (
            <div style={{ alignItems: "center", display: "flex", gap: "0.3rem", justifyContent: "center", marginTop: "0.1rem" }}>
            <TrackChip
            badHigh={false}
            glyph="🩸"
            label="Masquerade"
            value={Math.max(0, Math.min(100, Math.round(Number(worldState.masqueradeIntegrity))))}
            />
            <TrackChip
            badHigh={true}
            glyph="🕯"
            label="Heat"
            value={Math.max(0, Math.min(100, Math.round(Number(worldState.inquisitionHeat ?? 0))))}
            />
            </div>
        )}
        </div>
        ...
        style={{
            ...buttonStyle,
            color: openPanel === "skip" ? "rgba(232,221,200,0.9)" : buttonStyle.color,
        }}
        onClick={() => {
            if (isLoading) {
                setPanel("skip");
        ...
        >
        {isLoading ? <SpinnerRing size={15} tone="rgba(232,221,200,0.95)" /> : "\u00BB"}
        </button>
        </div>
        </>
    );
