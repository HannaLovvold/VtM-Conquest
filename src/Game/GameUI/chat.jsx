...
import { ACCENT, ACCENT_BRIGHT, TEXT, TEXT_BRIGHT } from "./theme";
...
const countryAccentColor = (name) => {
    const colors = ["#ef4444","#f97316","#eab308","#22c55e","#14b8a6",ACCENT,"#8b5cf6","#ec4899"];
    let h = 0;
...
.chat-markdown strong { color: rgba(232,221,200,0.95); }
.chat-markdown em { color: rgba(232,221,200,0.75); }
.chat-markdown blockquote { border-left: 2px solid rgba(138,3,3,0.6); margin: 0.5rem 0; padding-left: 0.75rem; color: rgba(232,221,200,0.6); }
`;
...
const MessageBubble = ({ msg }) => {
    ...
    return (
        ...
        {!isPlayer && msg.time && (
            <span style={{ fontSize: "0.65rem", color: "rgba(232,221,200,0.3)", marginTop: "0.25rem", display: "block" }}>
...
const ReactionBubble = ({ country, emoji, flag, code }) => {
    ...
    return (
        ...
        style={{
            ...
            : "1px solid rgba(232,221,200,0.15)",
...
const TypingBubble = ({ speaker, code }) => {
    ...
    return (
        ...
        <span style={{ fontSize: "0.7rem", color: "rgba(232,221,200,0.4)", marginBottom: "0.25rem" }}>{flag} {speaker}</span>
        <div style={{ padding: "0.6rem 0.85rem", borderRadius: "12px 12px 12px 4px", backgroundColor: "rgba(232,221,200,0.08)", fontSize: "0.85rem" }}>
        <ThinkingDots />
        </div>
        </div>
    );
...
const CountryTile = ({ country, code, flag, isSelected, onToggle }) => {
    ...
    return (
        ...
        style={{
            ...
            borderRadius: "10px",
            border: isSelected
            ? "1px solid rgba(138,3,3,0.6)"
            : hovered
            ? "1px solid rgba(232,221,200,0.15)"
            : "1px solid rgba(232,221,200,0.07)",
            background: isSelected
            ? "rgba(138,3,3,0.18)"
            : hovered
            ? "rgba(232,221,200,0.07)"
            : "rgba(232,221,200,0.04)",
            cursor: "pointer",
        ...
        >
        {isSelected && (
            <div style={{ position: "absolute", top: "0.3rem", right: "0.3rem", width: "14px", height: "14px", borderRadius: "50%", background: ACCENT, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.55rem", color: TEXT_BRIGHT, fontWeight: 700 }}>✓</div>
        )}
        <span style={{ fontSize: "1.6rem", lineHeight: 1 }}>{flag}</span>
        <span style={{ fontSize: "0.72rem", color: "rgba(232,221,200,0.8)", textAlign: "center", lineHeight: 1.3 }}>{shortName}</span>
        </button>
    );
};
...
const CountrySelectorModal = ({ countries, loading, onStart, onCancel }) => {
    ...
    return (
        <div style={{ position: "absolute", inset: 0, backgroundColor: "rgba(10, 6, 5, 0.98)", borderRadius: "16px", display: "flex", flexDirection: "column", zIndex: 10 }}>
        <div style={{ padding: "1.1rem 1.25rem 0.6rem", flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
        <div style={{ fontWeight: 700, fontSize: "1.05rem", color: TEXT_BRIGHT }}>Start New Diplomatic Chat</div>
        <div style={{ fontSize: "0.78rem", color: "rgba(232,221,200,0.4)", marginTop: "0.2rem" }}>Select countries to invite to the conversation</div>
        </div>
        <button onClick={onCancel} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(232,221,200,0.5)", fontSize: "1.1rem", padding: "0.1rem 0.3rem", borderRadius: "6px", lineHeight: 1 }}
        onMouseEnter={e => { e.currentTarget.style.color = TEXT_BRIGHT; e.currentTarget.style.background = "rgba(232,221,200,0.08)"; }}
        onMouseLeave={e => { e.currentTarget.style.color = "rgba(232,221,200,0.5)"; e.currentTarget.style.background = "none"; }}>✕</button>
        </div>
        <div style={{ marginTop: "0.85rem", padding: "0.65rem 0.9rem", borderRadius: "10px", backgroundColor: "rgba(232,221,200,0.05)", border: "1px solid rgba(232,221,200,0.1)" }}>
        <div style={{ fontSize: "0.8rem", fontWeight: 600, color: "rgba(232,221,200,0.8)" }}>Selected Countries ({selected.length}):</div>
        <div style={{ fontSize: "0.78rem", color: "rgba(232,221,200,0.35)", marginTop: "0.2rem" }}>
        ...
        <span style={{ position: "absolute", left: "0.75rem", color: "rgba(232,221,200,0.35)", display: "flex", pointerEvents: "none" }}><SearchIcon /></span>
        <input type="text" placeholder="Search countries..." value={search} onChange={e => setSearch(e.target.value)}
        style={{ width: "100%", padding: "0.55rem 0.85rem 0.55rem 2.2rem", borderRadius: "10px", border: "1px solid rgba(232,221,200,0.12)", background: "rgba(232,221,200,0.05)", color: TEXT_BRIGHT, fontSize: "0.82rem", outline: "none", boxSizing: "border-box", fontFamily: "sans-serif" }}
        onFocus={e => e.target.style.borderColor = "rgba(138,3,3,0.5)"}
        onBlur={e => e.target.style.borderColor = "rgba(232,221,200,0.12)"} />
        </div>
        </div>
        ...
        {loading && <p style={{ gridColumn: "1/-1", color: "rgba(232,221,200,0.35)", fontSize: "0.82rem", fontStyle: "italic", textAlign: "center" }}>Loading countries…</p>}
        {filtered.map(c => (
        ...
        <div style={{ padding: "0.75rem 1rem", borderTop: "1px solid rgba(232,221,200,0.07)", display: "flex", gap: "0.5rem", flexShrink: 0 }}>
        <button onClick={onCancel} style={{ flex: 1, padding: "0.65rem", borderRadius: "10px", border: "1px solid rgba(232,221,200,0.12)", background: "rgba(232,221,200,0.06)", color: "rgba(232,221,200,0.8)", fontSize: "0.85rem", fontWeight: 500, cursor: "pointer", fontFamily: "sans-serif" }}
        onMouseEnter={e => e.currentTarget.style.background = "rgba(232,221,200,0.1)"}
        onMouseLeave={e => e.currentTarget.style.background = "rgba(232,221,200,0.06)"}>Cancel</button>
        <button onClick={() => selected.length > 0 && onStart(selected)} disabled={selected.length === 0}
        style={{ flex: 2, padding: "0.65rem", borderRadius: "10px", border: "none", background: selected.length > 0 ? ACCENT : "rgba(138,3,3,0.3)", color: TEXT_BRIGHT, fontSize: "0.85rem", fontWeight: 600, cursor: selected.length > 0 ? "pointer" : "not-allowed", fontFamily: "sans-serif" }}
        onMouseEnter={e => { if (selected.length > 0) e.currentTarget.style.background = ACCENT_BRIGHT; }}
        onMouseLeave={e => { if (selected.length > 0) e.currentTarget.style.background = ACCENT; }}>
...
const ConversationView = ({ chat, playerCountry, gameDate, onDelete, onBack, onMessagesUpdate }) => {
    ...
    useEffect(() => {
        ...
        return (
            <>
            <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", padding: "0.85rem 1rem", borderBottom: "1px solid rgba(232,221,200,0.07)", flexShrink: 0 }}>
            <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(232,221,200,0.6)", display: "flex", padding: "0.2rem", borderRadius: "6px" }}
            onMouseEnter={e => { e.currentTarget.style.color = TEXT_BRIGHT; e.currentTarget.style.background = "rgba(232,221,200,0.08)"; }}
            onMouseLeave={e => { e.currentTarget.style.color = "rgba(232,221,200,0.6)"; e.currentTarget.style.background = "none"; }}>
            <BackIcon />
            </button>
            <span style={{ flex: 1, fontWeight: 700, fontSize: "0.95rem", color: TEXT_BRIGHT, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            ...
            <button onClick={onBack} style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(232,221,200,0.45)", fontSize: "1rem", lineHeight: 1, padding: "0.25rem 0.3rem", borderRadius: "6px" }}
            onMouseEnter={e => { e.currentTarget.style.color = TEXT_BRIGHT; e.currentTarget.style.background = "rgba(232,221,200,0.08)"; }}
            onMouseLeave={e => { e.currentTarget.style.color = "rgba(232,221,200,0.45)"; e.currentTarget.style.background = "none"; }}>✕</button>
            </div>
            ...
            {messages.length === 0 && !isLoading && (
                <p style={{ fontSize: "0.85rem", color: "rgba(232,221,200,0.35)", fontStyle: "italic", textAlign: "center", marginTop: "2rem" }}>
            ...
            {phase === "pending" && !isLoading && pendingCountry ? (
                <div style={{ padding: "0.75rem 1rem 0.9rem", borderTop: "1px solid rgba(232,221,200,0.07)", backgroundColor: "rgba(0,0,0,0.15)", flexShrink: 0 }}>
                <p style={{ margin: "0 0 0.55rem 0", fontSize: "0.78rem", color: "rgba(232,221,200,0.35)", textAlign: "center" }}>
                <CountryTurnLabel country={pendingCountry} remaining={remainingQueue.length} />
                </p>
                <div style={{ display: "flex", gap: "0.5rem" }}>
                <button
                onClick={handleSpeakInstead}
                style={{ flex: 1, padding: "0.58rem 0.7rem", borderRadius: "10px", border: "1px solid rgba(232,221,200,0.12)", background: "rgba(232,221,200,0.06)", color: "rgba(232,221,200,0.8)", fontSize: "0.9rem", fontWeight: 600, cursor: "pointer", fontFamily: "sans-serif", transition: "all 0.12s ease" }}
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(232,221,200,0.11)"; e.currentTarget.style.borderColor = "rgba(232,221,200,0.2)"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "rgba(232,221,200,0.06)"; e.currentTarget.style.borderColor = "rgba(232,221,200,0.12)"; }}
                >Speak</button>
                <button
                onClick={handleLetSpeak}
                style={{ flex: 2, padding: "0.58rem 0.7rem", borderRadius: "10px", border: "1px solid rgba(138,3,3,0.3)", background: "rgba(138,3,3,0.12)", color: "rgba(232,221,200,0.88)", fontSize: "0.82rem", fontWeight: 600, cursor: "pointer", fontFamily: "sans-serif", transition: "all 0.12s ease" }}
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(138,3,3,0.24)"; e.currentTarget.style.borderColor = "rgba(138,3,3,0.55)"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "rgba(138,3,3,0.12)"; e.currentTarget.style.borderColor = "rgba(138,3,3,0.3)"; }}
                >Let {pendingCountry.name} speak →</button>
                </div>
                </div>
            ) : phase === "player" && !isLoading ? (
                <div style={{ padding: "1rem", borderTop: "1px solid rgba(232,221,200,0.1)", display: "flex", alignItems: "center", gap: "0.5rem", flexShrink: 0 }}>
                <textarea
                ...
                style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.2)", border: "1px solid rgba(232,221,200,0.15)", borderRadius: "10px", color: TEXT_BRIGHT, fontSize: "0.875rem", padding: "0.6rem 0.75rem", resize: "none", outline: "none", fontFamily: "sans-serif", lineHeight: "1.5", overflowY: "hidden", transition: "border-color 0.2s" }}
                onFocus={e => e.target.style.borderColor = "rgba(138,3,3,0.6)"}
                onBlur={e => e.target.style.borderColor = "rgba(232,221,200,0.15)"}
                />
                <button onClick={handlePlayerSubmit} disabled={!playerInput.trim()}
                style={{ backgroundColor: playerInput.trim() ? ACCENT : "rgba(138,3,3,0.3)", border: "none", borderRadius: "10px", width: "2.5rem", height: "2.5rem", display: "flex", alignItems: "center", justifyContent: "center", cursor: playerInput.trim() ? "pointer" : "not-allowed", flexShrink: 0, fontSize: "1rem", transition: "background-color 0.2s" }}
                onMouseEnter={e => { if (playerInput.trim()) e.currentTarget.style.backgroundColor = ACCENT_BRIGHT; }}
                onMouseLeave={e => { if (playerInput.trim()) e.currentTarget.style.backgroundColor = ACCENT; }}
                >🚀</button>
                </div>
            ) : null}
            </>
...
const CountryTurnLabel = ({ country, remaining }) => {
    ...
    return (
        <>
        {flag} <strong style={{ color: "rgba(232,221,200,0.65)", fontWeight: 600 }}>{country.name}</strong> would like to respond
        {remaining > 0 && <span style={{ color: "rgba(232,221,200,0.22)" }}> · {remaining} more after</span>}
        </>
    );
};
...
const ChatListItem = ({ chat, onClick, onDelete, unread = false }) => {
    ...
    return (
        ...
        <button onClick={onClick} style={{ width: "100%", padding: "0.7rem 0.9rem", borderRadius: "10px", border: "1px solid rgba(232,221,200,0.07)", background: hovered ? "rgba(232,221,200,0.07)" : "rgba(232,221,200,0.03)", display: "flex", alignItems: "center", gap: "0.75rem", cursor: "pointer", transition: "background 0.15s", fontFamily: "sans-serif", textAlign: "left" }}>
        ...
        {unread && <div style={{ width: "0.5rem", height: "0.5rem", borderRadius: "50%", background: ACCENT_BRIGHT }} />}
        </div>
        <div style={{ fontSize: "1.3rem", flexShrink: 0, lineHeight: 1 }}>{flags}</div>
        <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: "0.82rem", fontWeight: unread ? 700 : 600, color: unread ? TEXT_BRIGHT : "rgba(232,221,200,0.9)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{names}{unread && <span style={{ fontWeight: 400, fontSize: "0.7rem", color: ACCENT_BRIGHT, marginLeft: "0.4rem" }}>new</span>}</div>
        <div style={{ fontSize: "0.75rem", color: unread ? "rgba(232,221,200,0.6)" : "rgba(232,221,200,0.35)", marginTop: "0.15rem", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{preview}</div>
        </div>
        </button>
        {hovered && (
...
const ChatPanel = ({ isOpen, onClose, requestedCountry, onConsumeRequest }) => {
    ...
    useEffect(() => {
        ...
        return (
            ...
            <div style={{ position: "fixed", bottom: isOpen ? "4.25rem" : "-40rem", left: "0rem", width: "26.25rem", maxWidth: "calc(100vw - 1rem)", height: "min(calc(100vh - 9rem), max(calc(100vh - 33rem), 30rem))", minHeight: "10rem", backgroundColor: "rgba(12, 8, 8, 0.95)", backdropFilter: "blur(8px)", borderRadius: "16px", border: "1px solid rgba(232,221,200,0.1)", boxShadow: "-4px 0 24px rgba(0,0,0,0.4),inset 0 1px 0 rgba(255,255,255,0.06)", zIndex: 9998, overflow: "hidden", transition: "bottom 0.35s cubic-bezier(0.4,0,0.2,1),opacity 0.35s ease", opacity: isOpen ? 1 : 0, pointerEvents: isOpen ? "auto" : "none", fontFamily: "sans-serif", color: TEXT, display: "flex", flexDirection: "column" }}>
            ...
            {activeChat && Array.isArray(activeChat.countries) && activeChat.countries.length > 0 ? (
                ...
                <>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "1rem 1.25rem 0.75rem", borderBottom: "1px solid rgba(232,221,200,0.07)", flexShrink: 0 }}>
                <span style={{ fontWeight: 700, fontSize: "1rem" }}>Diplomatic Chats</span>
                <button onClick={onClose} style={{ background: "none", border: "none", color: "rgba(232,221,200,0.5)", cursor: "pointer", fontSize: "1.1rem", lineHeight: 1, padding: "0.15rem 0.3rem", borderRadius: "6px" }}
                onMouseEnter={e => { e.currentTarget.style.color = TEXT_BRIGHT; e.currentTarget.style.background = "rgba(232,221,200,0.08)"; }}
                onMouseLeave={e => { e.currentTarget.style.color = "rgba(232,221,200,0.5)"; e.currentTarget.style.background = "none"; }}>✕</button>
                </div>
                <div style={{ flex: 1, overflowY: "auto", scrollbarWidth: "none", padding: "0.75rem 1rem", display: "flex", flexDirection: "column", gap: "0.4rem" }}>
                {openChats.length === 0 ? (
                    <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(232,221,200,0.25)", fontSize: "0.82rem", fontStyle: "italic", textAlign: "center", padding: "2rem" }}>
                ...
                <div style={{ padding: "0.75rem 1rem", borderTop: "1px solid rgba(232,221,200,0.07)", flexShrink: 0 }}>
                <button onClick={() => setShowSelector(true)} style={{ width: "100%", padding: "0.7rem", borderRadius: "10px", border: "1px solid rgba(232,221,200,0.12)", background: "rgba(232,221,200,0.07)", color: "rgba(232,221,200,0.85)", fontSize: "0.85rem", fontWeight: 500, cursor: "pointer", fontFamily: "sans-serif" }}
                onMouseEnter={e => e.currentTarget.style.background = "rgba(232,221,200,0.12)"}
                onMouseLeave={e => e.currentTarget.style.background = "rgba(232,221,200,0.07)"}>Start New Chat</button>
                </div>
                </>
            )}
            </div>
...
const Chat = ({ hovered, setHovered, isOpen, onToggle }) => {
    ...
    useEffect(() => {
        ...
        return (
            ...
            <button title="Chat" style={{ width: "3.3rem", height: "3.3rem", borderRadius: "10px", border: hovered ? "1px solid rgba(232,221,200,0.2)" : isOpen ? "1px solid rgba(138,3,3,0.5)" : "1px solid rgba(232,221,200,0.1)", background: isOpen ? "linear-gradient(145deg,rgba(138,3,3,0.4),rgba(90,2,2,0.4))" : hovered ? "linear-gradient(145deg,rgba(46,31,25,0.95),rgba(24,15,12,0.95))" : "linear-gradient(145deg,rgba(34,22,18,0.95),rgba(17,10,8,0.95))", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.12s ease", boxShadow: hovered ? "inset 0 1px 0 rgba(255,255,255,0.1),0 2px 8px rgba(0,0,0,0.4)" : "inset 0 1px 0 rgba(255,255,255,0.06),inset 0 -1px 0 rgba(0,0,0,0.3),0 2px 6px rgba(0,0,0,0.35)", fontSize: "1.2rem", outline: "none", transform: hovered ? "translateY(-1px)" : "translateY(0)", color: TEXT_BRIGHT, fontFamily: "sans-serif", flexShrink: 0 }}
            onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
            onClick={() => setChatOpen(o => !o)}>
            <span style={{ position: "relative", display: "inline-flex" }}>
                💬
                {unseenCount > 0 && !isOpen && (
                    <span style={{ position: "absolute", top: "-0.55rem", right: "-0.8rem", minWidth: "1.05rem", height: "1.05rem", padding: "0 0.2rem", borderRadius: "999px", background: "#dc2626", border: "1px solid rgba(232,221,200,0.35)", color: TEXT_BRIGHT, fontSize: "0.62rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, boxShadow: "0 1px 4px rgba(0,0,0,0.5)" }}>
...
const Toolbar = memo(({ onOpenAdvisor, activePanel, onTogglePanel }) => {
    ...
    return (
        <div style={{ position: "fixed", bottom: "0.5rem", left: "0.5rem", height: "4rem", width: "8.75rem", gap: "0.75rem", padding: "0 0.1rem", backgroundColor: "rgba(12, 8, 8, 0.9)", backdropFilter: "blur(4px)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", color: TEXT, fontFamily: "sans-serif", borderRadius: "14px", border: "1px solid rgba(232,221,200,0.08)", boxShadow: "0 8px 24px rgba(0,0,0,0.5),inset 0 1px 0 rgba(255,255,255,0.05)" }}>
