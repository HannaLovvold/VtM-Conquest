...
import { SURFACE, BORDER, TEXT, TEXT_BRIGHT, FONT_UI, ACCENT, ACCENT_BRIGHT, ACCENT_SOFT } from "./theme";
    ...
    useEffect(() => {
        ...
        chartRef.current = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        backgroundColor: "rgba(12,8,8,0.95)",
                                     borderColor: "rgba(232,221,200,0.12)", borderWidth: 1,
                                     titleColor: "rgba(232,221,200,0.85)", bodyColor: "rgba(232,221,200,0.6)",
                ...
                scales: isCartesian ? {
                    x: { ticks: { color: "rgba(232,221,200,0.45)", font: { size: 10, family: "sans-serif" } }, grid: { color: "rgba(232,221,200,0.06)" }, border: { color: "rgba(232,221,200,0.08)" }, ...coloredConfig.options?.scales?.x },
                                     y: { ticks: { color: "rgba(232,221,200,0.45)", font: { size: 10, family: "sans-serif" }, callback: val => `${val}${isPercent ? "%" : ""}` }, grid: { color: "rgba(232,221,200,0.06)" }, border: { color: "rgba(232,221,200,0.08)" }, ...coloredConfig.options?.scales?.y },
                } : undefined,
            },
        });
    ...
    return (
        ...
        {legendItems.length > 0 && (
            ...
            {legendItems.map((item, i) => (
                <span key={i} style={{ display: "flex", alignItems: "center", gap: "0.35rem", fontSize: "0.72rem", color: "rgba(232,221,200,0.5)" }}>
...
const TabButton = ({ icon, label, active, onClick }) => (
    ...
    style={{
        ...
        color: active ? TEXT_BRIGHT : "rgba(232,221,200,0.55)",
        cursor: "pointer",
        display: "flex",
        fontFamily: FONT_UI,
        fontSize: "0.88rem",
...
const AdvisorPanel = ({ isAdvisorOpen, onClose, width, onResize }) => {
    ...
    return (
        ...
        <div style={{
            ...
            backgroundColor: "rgba(22, 15, 13, 0.95)", backdropFilter: "blur(8px)",
            ...
            zIndex: 10040, borderLeft: "1px solid rgba(232,221,200,0.1)",
            boxShadow: "-4px 0 24px rgba(0,0,0,0.4)",
            transition: "transform 0.35s cubic-bezier(0.4, 0, 0.2, 1)",
            display: "flex", flexDirection: "column",
            color: TEXT, fontFamily: FONT_UI, overflow: "hidden",
        }}>
            ...
            >
                <div style={{
                    ...
                    backgroundColor: isResizing
                        ? "rgba(192, 32, 32, 0.95)"
                        : handleHover ? "rgba(232,221,200,0.5)" : "rgba(232,221,200,0.22)",
                    transition: "background-color 0.15s",
                }} />
            </div>
        )}
        {/* Header: tabs to flip between the advisor chat and national stats. */}
        <div style={{ alignItems: "center", borderBottom: "1px solid rgba(232,221,200,0.1)", display: "flex", padding: "0 0.75rem 0 0.35rem" }}>
        ...
        {activeTab === "advisor" && (
            ...
            title="Clear chat"
            style={{ background: "none", border: "none", color: "rgba(232,221,200,0.4)", cursor: "pointer", fontSize: "1.35rem", lineHeight: 1, padding: 0, display: "flex", alignItems: "center" }}
            >🗑</button>
        )}
        ...
        {onClose && (
            ...
            onClick={onClose}
            title="Close advisor"
            style={{ background: "none", border: "none", color: "rgba(232,221,200,0.55)", cursor: "pointer", fontSize: "1.35rem", lineHeight: 1, padding: "0 0 0 0.5rem", display: "flex", alignItems: "center" }}
            >✕</button>
        )}
        </div>
        ...
        {messages.length === 0 && (
            <p style={{ fontSize: "0.85rem", color: "rgba(232,221,200,0.5)", marginTop: 0 }}>
        ...
        {messages.map((msg, i) => {
            ...
            return (
                ...
                {msg.role !== "user" && (
                    <span style={{ fontSize: "0.7rem", color: "rgba(232,221,200,0.4)", marginBottom: "0.25rem" }}>
                ...
                <div data-no-translate={msg.role === "user" || asWritten ? "" : undefined} dir={asWritten ? chatDir : undefined} style={{
                    ...
                    backgroundColor: msg.role === "user" ? ACCENT_BRIGHT : msg.role === "error" ? "rgba(239,68,68,0.2)" : "rgba(232,221,200,0.08)",
                ...
                {msg.time && msg.role !== "user" && (
                    <span style={{ fontSize: "0.65rem", color: "rgba(232,221,200,0.3)", marginTop: "0.25rem" }}>
                    {formatDate(msg.time)}
                    </span>
                )}
                </div>
        ...
        {isLoading && !(messages[messages.length - 1]?.role === "advisor" && messages[messages.length - 1]?.streaming) && (
            ...
            <span style={{ fontSize: "0.7rem", color: "rgba(232,221,200,0.4)" }}>🧭 Advisor</span>
        ...
        {/* Input */}
        <div style={{ padding: "1rem", borderTop: "1px solid rgba(232,221,200,0.1)", display: "flex", alignItems: "center", gap: "0.5rem" }}>
        <textarea
        ref={inputRef}
        ...
        style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.2)", border: "1px solid rgba(232,221,200,0.15)", borderRadius: "10px", color: TEXT, fontSize: "0.875rem", padding: "0.6rem 0.75rem", resize: "none", outline: "none", fontFamily: "sans-serif", lineHeight: "1.5", overflowY: "auto", scrollbarWidth: "none", transition: "border-color 0.2s" }}
        onFocus={e => e.target.style.borderColor = ACCENT_SOFT}
        onBlur={e => e.target.style.borderColor = "rgba(232,221,200,0.15)"}
        />
        <button
        onClick={handleSend} disabled={isLoading || !input.trim()}
        style={{ backgroundColor: isLoading || !input.trim() ? "rgba(138, 3, 3, 0.4)" : ACCENT_BRIGHT, border: "none", borderRadius: "10px", width: "2.5rem", height: "2.5rem", display: "flex", alignItems: "center", justifyContent: "center", cursor: isLoading || !input.trim() ? "not-allowed" : "pointer", flexShrink: 0, fontSize: "1rem", transition: "background-color 0.2s" }}
        onMouseEnter={e => { if (!isLoading && input.trim()) e.currentTarget.style.backgroundColor = "#8a0303"; }}
        onMouseLeave={e => { if (!isLoading && input.trim()) e.currentTarget.style.backgroundColor = "#c02020"; }}
        >🚀</button>
        </div>
        </div>
        </div>
...
.advisor-markdown strong { color: rgba(232,221,200,0.95); }
.advisor-markdown em { color: rgba(232,221,200,0.75); }
...
.advisor-markdown h1, .advisor-markdown h2, .advisor-markdown h3 { margin: 0.75rem 0 0.25rem; font-size: 0.95rem; color: rgba(232,221,200,0.9); }
.advisor-markdown blockquote { border-left: 2px solid rgba(138, 3, 3, 0.6); margin: 0.5rem 0; padding-left: 0.75rem; color: rgba(232,221,200,0.6); }
`;
