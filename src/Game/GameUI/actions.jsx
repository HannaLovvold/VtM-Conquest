...
import { ACCENT, ACCENT_BRIGHT, BORDER, TEXT, TEXT_BRIGHT } from "./theme";
...
const ActionItem = ({ action, onDelete }) => {
    ...
    return (
        ...
        style={{
            alignItems: "center",
            backgroundColor: hovered ? "rgba(232,221,200,0.08)" : "rgba(232,221,200,0.05)",
            border: "1px solid rgba(232,221,200,0.07)",
            borderRadius: "10px",
        ...
        {showTitle && (
            <div style={{ color: TEXT_BRIGHT, fontSize: "0.78rem", fontWeight: 700, marginBottom: "0.15rem" }}>
            {normalized.title}
            </div>
        )}
        <div style={{ color: "rgba(232,221,200,0.82)", fontSize: "0.82rem", whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
        {label}
        </div>
        <div
        style={{
            color: "rgba(232,221,200,0.38)",
            fontSize: "0.68rem",
...
const SuggestionCard = ({ topic, onQueue, queuedIds }) => (
    ...
    >
    <div>
    <div style={{ color: TEXT_BRIGHT, fontSize: "0.8rem", fontWeight: 700 }}>{topic.title}</div>
    <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.76rem", lineHeight: "1.5", marginTop: "0.2rem" }}>
    {topic.description}
    </div>
    </div>
    ...
    {topic.actions.map((action) => {
        ...
        return (
            ...
            style={{
                background: isQueued ? "rgba(34,197,94,0.12)" : "rgba(138,3,3,0.12)",
                border: isQueued ? "1px solid rgba(74,222,128,0.35)" : "1px solid rgba(138,3,3,0.24)",
                borderRadius: "10px",
            ...
            <div style={{ color: "rgba(232,221,200,0.62)", fontSize: "0.74rem", lineHeight: "1.45", marginTop: "0.18rem" }}>
            {action.text}
            </div>
            </button>
        );
...
const ActionsPanel = ({ isOpen, onClose, onOpenAdvisor }) => {
    ...
    return (
        ...
        onMouseEnter={(event) => {
            event.currentTarget.style.background = "rgba(138, 3, 3, 0.28)";
            event.currentTarget.style.borderColor = "rgba(138,3,3,0.65)";
        }}
        onMouseLeave={(event) => {
            event.currentTarget.style.background = "rgba(138, 3, 3, 0.15)";
            event.currentTarget.style.borderColor = "rgba(138,3,3,0.4)";
        }}
        >
        ...
        onMouseEnter={(event) => {
            event.currentTarget.style.background = "rgba(232,221,200,0.09)";
            event.currentTarget.style.borderColor = "rgba(232,221,200,0.18)";
        }}
        onMouseLeave={(event) => {
            event.currentTarget.style.background = "rgba(232,221,200,0.05)";
            event.currentTarget.style.borderColor = "rgba(232,221,200,0.12)";
        }}
        >
        ...
        {(hasRequestedSuggestions || isSuggesting || suggestions.length > 0) && (
            ...
            >
            {hasRequestedSuggestions && !isSuggesting && suggestions.length === 0 && (
                <p style={{ color: "rgba(232,221,200,0.35)", fontSize: "0.78rem", fontStyle: "italic", margin: 0 }}>
        ...
        {submittedActions.length === 0 && (
            <p style={{ color: "rgba(232,221,200,0.35)", fontSize: "0.8rem", fontStyle: "italic", margin: 0 }}>
        ...
        style={{
            ...
            borderTop: "1px solid rgba(232,221,200,0.07)",
            display: "flex",
            gap: "0.5rem",
        ...
        onFocus={(event) => {
            event.target.style.borderColor = "rgba(138,3,3,0.5)";
        }}
        onBlur={(event) => {
            event.target.style.borderColor = "rgba(232,221,200,0.12)";
        }}
        />
        <button
        type="button"
        ...
        style={{
            ...
            border: "none",
            borderRadius: "8px",
            color: inputValue.trim() ? "rgba(232,221,200,0.78)" : "rgba(232,221,200,0.35)",
        ...
        >
        {isImproving ? <SpinnerRing size={14} tone="rgba(232,221,200,0.9)" /> : <SparkleIcon />}
        </button>
        </div>
        ...
        style={{
            alignItems: "center",
            background: inputValue.trim() && !isSubmitting && !isImproving ? ACCENT : "rgba(138,3,3,0.3)",
            border: "none",
            borderRadius: "10px",
            color: TEXT_BRIGHT,
        ...
        onMouseEnter={(event) => {
            if (inputValue.trim() && !isSubmitting && !isImproving) {
                event.currentTarget.style.background = ACCENT_BRIGHT;
            }
        }}
        onMouseLeave={(event) => {
            if (inputValue.trim() && !isSubmitting && !isImproving) {
...
const Actions = ({ onOpenAdvisor, hovered, setHovered, isOpen, onToggle }) => {
    ...
    return (
        ...
        style={{
            alignItems: "center",
            background: isOpen
            ? "linear-gradient(145deg, rgba(138,3,3,0.4), rgba(88,2,2,0.4))"
            : hovered
            ? "linear-gradient(145deg, rgba(40,30,27,0.95), rgba(24,17,15,0.95))"
            : "linear-gradient(145deg, rgba(30,21,19,0.95), rgba(16,11,10,0.95))",
            border: hovered
            ? "1px solid rgba(232,221,200,0.2)"
            : isOpen
            ? "1px solid rgba(138,3,3,0.5)"
            : "1px solid rgba(232,221,200,0.1)",
            borderRadius: "10px",
            boxShadow: hovered
            ? "inset 0 1px 0 rgba(232,221,200,0.1), 0 2px 8px rgba(0,0,0,0.4)"
            : "inset 0 1px 0 rgba(232,221,200,0.06), inset 0 -1px 0 rgba(0,0,0,0.3), 0 2px 6px rgba(0,0,0,0.35)",
            color: TEXT_BRIGHT,
            cursor: "pointer",
            display: "flex",
            fontFamily: "sans-serif",
