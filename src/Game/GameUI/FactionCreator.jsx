...
const label = { color: "rgba(232,221,200,0.7)", fontSize: "0.78rem", fontWeight: 700, margin: "0.1rem 0 0.3rem" };
const field = {
...
const pill = (active) => ({
  background: active ? "rgba(138,3,3,0.28)" : "rgba(232,221,200,0.06)",
  border: `1px solid ${active ? "rgba(138,3,3,0.7)" : "rgba(232,221,200,0.1)"}`,
  borderRadius: 999,
  color: "#e8ddc8",
  cursor: "pointer",
  fontSize: "0.8rem",
...
const FactionCreator = ({ regionsGeojson, onCreate, onCancel, busy }) => {
  ...
  const [color, setColor] = useState("#8a0303");
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
      ...
      <div style={{ display: "flex", gap: "0.8rem", alignItems: "flex-end" }}>
        <div>
          ...
          <input
            ...
            style={{ width: 48, height: 34, background: "none", border: "1px solid rgba(232,221,200,0.16)", borderRadius: 8, cursor: "pointer" }}
          />
        </div>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            {flag ? (
              <img src={flag} alt="" style={{ width: 34, height: 22, objectFit: "contain", borderRadius: 3, border: "1px solid rgba(232,221,200,0.25)" }} />
            ) : (
      ...
      <div>
        ...
        {landless ? (
          ...
          <>
            <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.76rem", marginBottom: "0.3rem" }}>
              {regionIds.size} region{regionIds.size === 1 ? "" : "s"} claimed
            </div>
            <Suspense fallback={<div style={{ color: "rgba(232,221,200,0.5)", padding: "2rem 0", textAlign: "center" }}>Loading map…</div>}>
              <CountryPickerMap
                countryOptions={[]}
                onPickCountry={() => {}}
