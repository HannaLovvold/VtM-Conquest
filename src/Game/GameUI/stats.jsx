...
import { ACCENT_BRIGHT, ACCENT_FAINT, BORDER, TEXT, TEXT_FAINT } from "./theme";
...
const cardStyle = {
    backgroundColor: "rgba(232,221,200,0.045)",
...
const Bar = ({ value, color }) => (
    <div style={{ backgroundColor: "rgba(232,221,200,0.1)", borderRadius: "999px", height: "6px", overflow: "hidden" }}>
...
const EconomyCard = ({ label, value, sub, tone }) => (
    <div style={cardStyle}>
    <div style={{ color: TEXT_FAINT, fontSize: "0.62rem", fontWeight: 700, letterSpacing: "0.06em", marginBottom: "0.3rem", textTransform: "uppercase" }}>
    {label}
    </div>
    <div data-no-translate style={{ color: tone, fontSize: "1.05rem", fontWeight: 800 }}>{compactEconomyValue(value) || "—"}</div>
    {sub && <div style={{ color: "rgba(232,221,200,0.4)", fontSize: "0.68rem", marginTop: "0.15rem" }}>{sub}</div>}
    </div>
);
...
const StatsPane = ({ active }) => {
    ...
    return (
        ...
        {!targetCountry && (
            <p style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.85rem" }}>
        ...
        {targetCountry && (
            ...
            <div style={{ alignItems: "center", backgroundColor: ACCENT_FAINT, border: `1px solid ${BORDER}`, borderRadius: "10px", color: ACCENT_BRIGHT, display: "flex", flexShrink: 0, fontSize: "0.95rem", fontWeight: 800, height: "2.6rem", justifyContent: "center", overflow: "hidden", width: "2.6rem" }}>
            {flagUrl && !flagFailed ? (
                <img
                alt=""
                src={flagUrl}
            ...
            {sheet && (
                <>
                <div style={{ color: "rgba(232,221,200,0.7)", fontSize: "0.76rem", marginTop: "0.15rem" }}>
                {[sheet.capital, sheet.continent].filter(Boolean).join(" · ")}
                </div>
                {sheet.government && (
                    <div style={{ color: TEXT_FAINT, fontSize: "0.72rem", marginTop: "0.1rem" }}>
                    {sheet.government}
                    </div>
                )}
                {sheet.leader && (
            ...
            {state.status !== "loading" && (
                ...
                style={{ background: "none", border: "none", color: "rgba(232,221,200,0.4)", cursor: "pointer", fontSize: "1rem", padding: 0 }}
                >↻</button>
            )}
            </div>

            {state.status === "loading" && (
                <p style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.82rem", marginTop: "1rem" }}>
            ...
            {state.status === "error" && (
                ...
                <button
                onClick={() => loadSheet({ force: true })}
                style={{ background: "none", border: "none", color: ACCENT_BRIGHT, cursor: "pointer", display: "block", fontSize: "0.8rem", fontWeight: 700, marginTop: "0.4rem", padding: 0 }}
                >Try again</button>
                </div>
            )}
            ...
            {sheet && state.status === "ready" && (
                ...
                <span style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.68rem", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase" }}>
                ⚠ National stability
                </span>
                ...
                {INDEX_ROWS.map((row) => {
                    ...
                    return (
                        ...
                        <span style={{ color: "rgba(232,221,200,0.8)", fontSize: "0.76rem" }}>
                        {row.icon} {row.label}
                        </span>
                ...
                <EconomyCard label="GDP/capita" value={sheet.economy?.gdpPerCapita} sub={sheet.economy?.currency} tone={TEXT} />
                ...
                <div style={{ color: "rgba(232,221,200,0.7)", fontSize: "0.74rem", marginBottom: "0.5rem" }}>
                GDP breakdown
                </div>
                ...
                {breakdown.map((part) => (
                    <span key={part.key} style={{ alignItems: "center", color: "rgba(232,221,200,0.6)", display: "flex", fontSize: "0.68rem", gap: "0.3rem" }}>
            ...
            <p style={{ color: "rgba(232,221,200,0.35)", fontSize: "0.7rem", marginTop: "1rem" }}>
