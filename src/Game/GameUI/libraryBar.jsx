...
import { ACCENT, TEXT, TEXT_BRIGHT } from "./theme";
...
const surfaceStyle = {
  background:
    "linear-gradient(180deg, rgba(10, 6, 5, 0.97) 0%, rgba(8, 5, 4, 0.94) 100%)",
  border: "1px solid rgba(232,221,200,0.08)",
...
const actionButtonStyle = {
  alignItems: "center",
  background: "rgba(232,221,200,0.06)",
  border: "1px solid rgba(232,221,200,0.08)",
  borderRadius: "999px",
...
const buildScenarioEditorState = (details) => {
  ...
  return {
    accentColor: scenario.accentColor ?? ACCENT,
...
const buildGameEditorState = (details) => {
  ...
  return {
    accentColor: gameMeta.accentColor ?? ACCENT,
    country: game.country ?? "",
    ...
    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem", marginBottom: "0.85rem" }}>
      {badges.map((badge) => (
        <span
          key={badge}
          style={{
            background: "rgba(232,221,200,0.14)",
            borderRadius: "999px",
...
const PromptSectionEditor = ({
  ...
  return (
    ...
    >
        ...
        {PROMPT_SECTION_DEFINITIONS.map((section) => (
          <button
            ...
            style={{
              ...actionButtonStyle,
              background:
                section.key === currentSection.key ? "rgba(138,3,3,0.28)" : "rgba(232,221,200,0.05)",
              borderColor:
                section.key === currentSection.key ? "rgba(138,3,3,0.42)" : "rgba(232,221,200,0.08)",
              minHeight: "2rem",
              padding: "0 0.8rem",
            }}
            type="button"
      ...
      <div style={{ color: "rgba(232,221,200,0.58)", fontSize: "0.82rem", marginBottom: "0.75rem" }}>
...
const ScenarioCard = ({ onClone, onEdit, onPlay, onSelect, scenario, selected }) => {
  ...
  return (
    <div
      style={{
        ...surfaceStyle,
        borderColor: selected ? `${scenario.accentColor}66` : "rgba(232,221,200,0.08)",
        borderRadius: "24px",
        flex: "0 0 21rem",
        minHeight: "15rem",
        overflow: "hidden",
    ...
    >
      ...
      >
        <div>
          <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between" }}>
            <div style={{ alignItems: "center", display: "flex", flexWrap: "wrap", gap: "0.45rem" }}>
              <span
                style={{
                  background: selected ? `${scenario.accentColor}66` : "rgba(232,221,200,0.12)",
                  border: "1px solid rgba(232,221,200,0.15)",
                  borderRadius: "999px",
                  color: "rgba(232,221,200,0.94)",
                  display: "inline-flex",
                  fontSize: "0.69rem",
                  fontWeight: 700,
              ...
              {isBuiltIn && (
                <span
                  style={{
                    background: "rgba(232,221,200,0.18)",
            ...
            <span style={{ color: "rgba(232,221,200,0.7)", fontSize: "0.74rem" }}>
        ...
        <div>
          <div style={{ color: "rgba(232,221,200,0.68)", fontSize: "0.8rem", marginBottom: "0.7rem" }}>
            {scenario.subtitle}
          </div>
...
const GameCard = ({ active, game, onActivate, onClone, onEdit }) => {
  ...
  return (
    <div
      style={{
        ...surfaceStyle,
        borderColor: active ? `${game.accentColor}66` : "rgba(232,221,200,0.08)",
        borderRadius: "24px",
        flex: "0 0 21rem",
        minHeight: "14rem",
        overflow: "hidden",
      ...
      >
        <div>
          <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between" }}>
            <span
              style={{
                background: active ? `${game.accentColor}66` : "rgba(232,221,200,0.12)",
                border: "1px solid rgba(232,221,200,0.15)",
                borderRadius: "999px",
                color: "rgba(232,221,200,0.94)",
                display: "inline-flex",
            ...
            <span style={{ color: "rgba(232,221,200,0.72)", fontSize: "0.76rem" }}>
              {game.scenarioName}
            </span>
          </div>

          <div style={{ marginTop: "2rem" }}>
            <div style={{ color: TEXT_BRIGHT, fontSize: "1.5rem", fontWeight: 800, letterSpacing: "-0.03em" }}>
              {game.name}
            </div>
            <div style={{ color: "rgba(232,221,200,0.72)", fontSize: "0.92rem", marginTop: "0.45rem" }}>
              {game.country || "No player country"} / {game.currentDate || "No date"} / Round {game.round || 1}
            </div>
            <div style={{ color: "rgba(232,221,200,0.58)", fontSize: "0.84rem", marginTop: "0.5rem", lineHeight: 1.45 }}>
        ...
        <div>
          <div style={{ color: "rgba(232,221,200,0.68)", fontSize: "0.8rem", marginBottom: "0.75rem" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.55rem" }}>
            <button
              ...
              style={{
                ...actionButtonStyle,
                background: active ? "rgba(232,221,200,0.16)" : `${game.accentColor}cc`,
                borderColor: active ? "rgba(232,221,200,0.22)" : `${game.accentColor}dd`,
                color: TEXT_BRIGHT,
                flex: 1,
              }}
              type="button"
...
const MenuRow = ({ children, emptyText, title }) => (
  <div style={{ marginBottom: "1.7rem" }}>
    <div style={{ color: "rgba(232,221,200,0.88)", fontSize: "1.02rem", fontWeight: 800, letterSpacing: "-0.01em", marginBottom: "0.7rem" }}>
      {title}
    </div>
    {React.Children.count(children) > 0 ? (
      ...
      <div style={{ color: "rgba(232,221,200,0.45)", fontSize: "0.85rem", padding: "0.4rem 0 0.6rem" }}>
...
const CreateScenarioTile = ({ busy, onCreate }) => (
  <button
    ...
    style={{
      alignItems: "center",
      background: "rgba(232,221,200,0.03)",
      border: "2px dashed rgba(232,221,200,0.24)",
      borderRadius: "24px",
    ...
    {sections.map((sectionKey) => (
      <button
        ...
        style={{
          ...actionButtonStyle,
          background:
            currentSection === sectionKey ? "rgba(138,3,3,0.28)" : "rgba(232,221,200,0.05)",
          borderColor:
            currentSection === sectionKey ? "rgba(138,3,3,0.42)" : "rgba(232,221,200,0.08)",
          minHeight: "2rem",
          padding: "0 0.8rem",
        }}
        type="button"
  ...
  return (
    ...
    >
      <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between", marginBottom: "1rem" }}>
        <div>
          <div style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.72rem", letterSpacing: "0.08em", textTransform: "uppercase" }}>
        ...
        <button
          onClick={onClose}
          style={{ ...actionButtonStyle, background: "rgba(232,221,200,0.04)", minWidth: "2.35rem", padding: 0 }}
          type="button"
        >
          X
        </button>
      ...
      {editorSection === "overview" && (
        <div style={{ background: "rgba(232,221,200,0.03)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: "18px", marginBottom: "0.95rem", padding: "0.9rem" }}>
          ...
          <div style={{ display: "grid", gap: "0.8rem", gridTemplateColumns: "repeat(2, minmax(0, 1fr))" }}>
            ...
            {kind === "scenario" && (
              <div style={{ gridColumn: "1 / -1" }}>
                  ...
                  {UNIT_TYPES.map((unitType) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...actionButtonStyle,
                          background: checked ? "rgba(138,3,3,0.3)" : "rgba(232,221,200,0.04)",
                          borderColor: checked ? "rgba(138,3,3,0.5)" : "rgba(232,221,200,0.1)",
                          minHeight: "2rem",
                          padding: "0 0.7rem",
                        }}
                      >
                ...
                <div style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.72rem", marginTop: "0.4rem" }}>
        ...
        <div style={{ background: "rgba(232,221,200,0.03)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: "18px", marginBottom: "0.95rem", padding: "0.9rem" }}>
          <div style={{ display: "grid", gap: "0.7rem" }}>
            {Object.entries(kind === "scenario" ? scenarioAssetLabels : gameAssetLabels).map(([assetKey, label]) => {
              ...
              return (
                <div key={assetKey} style={{ alignItems: "center", background: "rgba(232,221,200,0.03)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: "14px", display: "flex", gap: "0.75rem", justifyContent: "space-between", padding: "0.72rem 0.78rem" }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: "0.9rem", fontWeight: 600 }}>{label}</div>
                    <div style={{ color: "rgba(232,221,200,0.58)", fontSize: "0.78rem", marginTop: "0.15rem" }}>
                      {hasOwnAsset
                        ? isCoverAsset
                          ? kind === "scenario"
                  ...
                  <div style={{ display: "flex", gap: "0.45rem" }}>
                    ...
                    <button
                      ...
                      style={{
                        ...actionButtonStyle,
                        background: "rgba(232,221,200,0.03)",
                        color: hasOwnAsset ? "rgba(232,221,200,0.92)" : "rgba(232,221,200,0.35)",
                      }}
      ...
      {editorSection === "bundles" && kind === "scenario" && (
        <div style={{ background: "rgba(232,221,200,0.03)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: "18px", marginBottom: "0.95rem", padding: "0.9rem" }}>
          <div style={{ color: "rgba(232,221,200,0.58)", fontSize: "0.82rem", lineHeight: 1.5, marginBottom: "0.85rem" }}>
            Download the scenario as one self-contained file — custom map geometry, cities and basemap all travel with it, ready to share or re-import. The <strong>.zip</strong> carries a custom basemap as a real image file (smaller); the <strong>JSON</strong> packs everything into one text file.
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.55rem" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: "0.55rem" }}>
        <button
          onClick={onSave}
          style={{ ...actionButtonStyle, background: `${record.accentColor}cc`, borderColor: `${record.accentColor}dd`, color: TEXT_BRIGHT, minWidth: "7.2rem" }}
          type="button"
        >
          {isBusy ? "Saving..." : "Save"}
        </button>
        {kind === "scenario" && onOpenMapEditor && (
          <button
            ...
            style={{ ...actionButtonStyle, background: "rgba(138,3,3,0.24)", borderColor: "rgba(138,3,3,0.38)", color: TEXT_BRIGHT, minWidth: "9rem" }}
            type="button"
          >
            🗺️ Open Map Editor
          </button>
...
const LibraryTopBar = () => {
  ...
  const startGameForFaction = async (scenario, faction, difficulty) => {
    ...
    try {
      ...
      const hexColor = /^#[0-9a-fA-F]{6}$/.test(faction.color) ? faction.color : ACCENT;
  ...
  const handleShutdownServer = async () => {
    if (!window.confirm("Shut down the VtM Conquest server? The game stops for everyone connected to it.")) {
      return;
    }
    try {
  ...
  // "Your Scenarios": ones the player made or edited themselves. The hidden
  // stock built-in only counts once it has actually been touched.
  const yourScenarios = useMemo(
    () => scenarios.filter(
      (scenario) => scenario.id !== "default" || scenario.updatedAt !== scenario.createdAt,
    ),
    [scenarios],
  );
    ...
    <>
        ...
        >
          ...
          {!import.meta.env.VITE_OH_WEB && (
            <button
              ...
              title="Exit: shut down the VtM Conquest server"
              type="button"
              style={{
                ...actionButtonStyle,
                ...surfaceStyle,
      ...
      {serverDown && (
        ...
        >
          ...
          <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.85rem", maxWidth: "22rem" }}>
      ...
      {isMapEditorOpen && (
        <div style={{ position: "fixed", inset: 0, zIndex: 10050 }}>
          <Suspense
            fallback={
              <div style={{ position: "fixed", inset: 0, background: "#0a0605", color: TEXT, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "sans-serif" }}>
                Loading map editor…
              </div>
            }
          >
      ...
      {countryPicker && (
        ...
        >
          <div
            ...
            style={{ ...surfaceStyle, borderRadius: 16, width: difficultyPick ? "min(440px, 92vw)" : "min(640px, 92vw)", maxHeight: "80vh", display: "flex", flexDirection: "column", padding: "1rem", color: TEXT, fontFamily: "sans-serif", overflow: difficultyPick ? "visible" : "auto" }}
          >
            {difficultyPick ? (
              <>
                <div style={{ fontWeight: 800, fontSize: "1rem" }}>Choose your difficulty</div>
                <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.75rem", margin: "0.15rem 0 0.7rem" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.5rem", overflowY: "auto" }}>
                  {DIFFICULTY_LEVELS.map((level) => (
                    ...
                    >
                      ...
                      <span style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.68rem", textAlign: "center" }}>{level.blurb}</span>
                    </button>
                  ))}
                </div>
              ...
              <>
                ...
                <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.75rem", margin: "0.15rem 0 0.6rem" }}>
                  ...
                  <div style={{ display: "flex", gap: "0.4rem", marginBottom: "0.7rem" }}>
                    <button
                      ...
                      style={{
                        ...
                        flex: 1,
                        fontWeight: 700,
                        background: pickerTab === "country" ? "rgba(138,3,3,0.28)" : "rgba(232,221,200,0.05)",
                        borderColor: pickerTab === "country" ? "rgba(138,3,3,0.7)" : undefined,
                      }}
                    >
                      Pick a country
                    </button>
                    <button
                      ...
                      style={{
                        ...
                        flex: 1,
                        fontWeight: 700,
                        background: pickerTab === "faction" ? "rgba(138,3,3,0.28)" : "rgba(232,221,200,0.05)",
                        borderColor: pickerTab === "faction" ? "rgba(138,3,3,0.7)" : undefined,
                      }}
                    >
                      Create a faction
                    </button>
                ...
                {pickerTab === "faction" && !playGameId ? (
                  ...
                  <>
                    <button
                      type="button"
                      onClick={() => pickCountry("")}
                      style={{ ...actionButtonStyle, justifyContent: "flex-start", background: "rgba(138,3,3,0.18)", marginBottom: "0.4rem" }}
                    >
                      {playGameId ? "Keep scenario default" : "Scenario default"}
                    </button>
                    <Suspense
                      fallback={
                        <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.85rem", padding: "3rem 0", textAlign: "center" }}>
                          Loading map…
                        </div>
                      }
                    >
      ...
      {menuOpen && (
        <div
          style={{
            background:
              "radial-gradient(circle at 12% -4%, rgba(138,3,3,0.16), transparent 42%), " +
              "radial-gradient(circle at 88% 110%, rgba(192,32,32,0.10), transparent 46%), " +
              "linear-gradient(180deg, #0a0605 0%, #070404 100%)",
            color: TEXT,
            display: "flex",
            flexDirection: "column",
            fontFamily: "sans-serif",
        ...
        >
          <div
            style={{
              alignItems: "center",
              borderBottom: "1px solid rgba(232,221,200,0.08)",
              display: "grid",
              flexShrink: 0,
          ...
          >
            <div style={{ alignItems: "center", display: "flex", gap: "0.8rem", minWidth: 0 }}>
              <div style={{ alignItems: "center", background: "rgba(232,221,200,0.06)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: "999px", display: "flex", flexShrink: 0, height: "2.65rem", justifyContent: "center", overflow: "hidden", width: "2.65rem" }}>
                <img alt="VtM Conquest" src="/logo.png" style={{ height: "1.7rem", width: "1.7rem" }} />
              </div>
              {!isMobile && (
                <div style={{ color: TEXT_BRIGHT, fontSize: "1.05rem", fontWeight: 800, letterSpacing: "-0.03em" }}>
                  VtM Conquest
                </div>
              )}
            </div>
            ...
            <div style={{ alignItems: "center", display: "flex", gap: "0.55rem", justifyContent: "center", justifySelf: "center" }}>
              {["games", "scenarios"].map((tab) => (
                <button
                  ...
                  style={{
                    ...actionButtonStyle,
                    background: activeTab === tab ? "rgba(138,3,3,0.24)" : "rgba(232,221,200,0.05)",
                    borderColor: activeTab === tab ? "rgba(138,3,3,0.38)" : "rgba(232,221,200,0.08)",
            ...
            {activeTab === "games" ? (
              loaded && games.length === 0 ? (
                <div style={{ alignItems: "center", display: "flex", flexDirection: "column", justifyContent: "center", minHeight: "60vh", textAlign: "center" }}>
                  ...
                  <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.95rem", lineHeight: 1.6, margin: "0.6rem 0 1.6rem", maxWidth: "26rem" }}>
                    Start a new game from one of your scenarios.
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.7rem", justifyContent: "center" }}>
                    <button
                      type="button"
                      style={{ ...actionButtonStyle, background: "rgba(138,3,3,0.3)", borderColor: "rgba(138,3,3,0.55)", minHeight: "2.8rem", padding: "0 1.4rem" }}
      ...
      {!loaded && (
        <div style={{ color: "rgba(232,221,200,0.6)", fontSize: "0.82rem", position: "fixed", right: "1.25rem", top: "4.35rem", zIndex: 10028 }}>
