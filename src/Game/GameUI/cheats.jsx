...
import { ACCENT_SOFT, BORDER, TEXT } from "./theme";
...
const CheatsPanel = ({ open, onClose, onOpenForces }) => {
    ...
    const header = (title, subtitle) => (
        <div style={{ borderBottom: "1px solid rgba(232,221,200,0.08)", marginBottom: "0.7rem", paddingBottom: "0.6rem" }}>
        ...
        {subtitle && <div style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.74rem", marginTop: "0.2rem" }}>{subtitle}</div>}
        </div>
    );
    ...
    return (
        <>
        {clickMode && (
            <div style={{ alignItems: "center", display: "flex", gap: "0.6rem", background: "rgba(12,8,8,0.97)", border: "1px solid rgba(138,3,3,0.5)", borderRadius: 12, boxShadow: "0 6px 24px rgba(0,0,0,0.5)", color: TEXT, fontFamily: "sans-serif", fontSize: "0.85rem", left: "50%", padding: "0.6rem 0.9rem", position: "fixed", top: PANEL_TOP, transform: "translateX(-50%)", zIndex: 10070 }}>
        ...
        {!tool ? (
            ...
            {typeof onOpenForces === "function" && (
                ...
                <span style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.72rem", fontWeight: 500 }}>Open the Forces panel to spawn, move, and command units by hand.</span>
                </button>
            )}
            {TOOLS.map((entry) => (
                <button
                ...
                <span style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.72rem", fontWeight: 500 }}>{entry.subtitle}</span>
                </button>
            ))}
            </div>
            </>
        ...
        {status && !tool && (
            <div style={{ color: "rgba(232,221,200,0.9)", fontSize: "0.76rem", marginTop: "0.6rem" }}>{status}</div>
        )}
        </div>
        </>
    );
...
const ToolView = ({ tool, header, busy, status, game, polities, refresh, runBusy, beginClickMode, endClickMode, setStatus }) => {
    ...
    const statusLine = status && (
        <div style={{ color: status.startsWith("Failed") ? "#fca5a5" : "rgba(232,221,200,0.9)", fontSize: "0.76rem", marginTop: "0.6rem" }}>
        {status}
        </div>
    );
    ...
    if (tool === "master-ai") {
        return (
            ...
            <div style={{ color: "rgba(232,221,200,0.45)", fontSize: "0.72rem", marginTop: "0.5rem" }}>
    ...
    if (tool === "roll-back-turn") {
        ...
        return (
            ...
            <div style={{ color: "rgba(232,221,200,0.55)", fontSize: "0.76rem", marginBottom: "0.5rem" }}>
            ...
            {items === null && (
                <div style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.76rem" }}>Loading restore points…</div>
            )}
            {items !== null && snapshots.length === 0 && (
                <div style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.76rem" }}>
            ...
            {snapshots.map((snap, index) => {
                ...
                return (
                    <div key={snap.id} style={{ background: "rgba(232,221,200,0.04)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: 8, padding: "0.5rem 0.6rem" }}>
                    ...
                    <div style={{ color: "rgba(232,221,200,0.45)", fontSize: "0.68rem" }}>
    ...
    if (tool === "your-country") {
        return (
            ...
            {header(meta.title, meta.subtitle)}
            <div>
            <div style={{ color: "rgba(232,221,200,0.6)", fontSize: "0.78rem" }}>
    ...
    if (tool === "difficulty") {
        ...
        return (
            ...
            {DIFFICULTY_LEVELS.map((level) => (
                ...
                style={{
                    ...buttonStyle,
                    background: current === level.id ? "rgba(138, 3, 3, 0.35)" : buttonStyle.background,
                    border: current === level.id ? "1px solid rgba(138, 3, 3, 0.65)" : buttonStyle.border,
                    flexDirection: "column",
                    gap: "0.15rem",
    ...
    if (tool === "edit-country" || tool === "add-country") {
        ...
        return (
            ...
            <input style={{ ...inputStyle, width: "8rem" }} value={fields.color ?? ""} onChange={(event) => setFields({ ...fields, color: event.target.value })} placeholder="#8a0303" />
            <input
            type="color"
            value={hexToRgb(fields.color) ? (fields.color.startsWith("#") ? fields.color : `#${fields.color}`) : "#8a0303"}
    ...
    if (tool === "regions") {
        return (
            ...
            {fields.id && (
                <>
                <div style={{ color: "rgba(232,221,200,0.6)", fontSize: "0.76rem", marginTop: "0.6rem" }}>
    ...
    if (tool === "edit-feature") {
        ...
        return (
            ...
            {features.length === 0 && (
                <div style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.76rem", marginTop: "0.6rem" }}>
            ...
            {shown.map(({ feature, index }) => {
                ...
                return (
                    <div key={index} style={{ background: "rgba(232,221,200,0.04)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: 8, padding: "0.5rem 0.6rem" }}>
                    <div style={{ alignItems: "center", display: "flex", justifyContent: "space-between" }}>
    ...
    if (tool === "events") {
        ...
        return (
            ...
            {events.length === 0 && (
                <div style={{ color: "rgba(232,221,200,0.5)", fontSize: "0.76rem", marginTop: "0.6rem" }}>No events yet.</div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem", marginTop: "0.5rem", overflowY: "auto" }}>
            {shown.map((event) => {
                const isEditing = editingId === event.id;
                return (
                    <div key={event.id} style={{ background: "rgba(232,221,200,0.04)", border: "1px solid rgba(232,221,200,0.08)", borderRadius: 8, padding: "0.5rem 0.6rem" }}>
                    <div style={{ alignItems: "center", display: "flex", gap: "0.4rem", justifyContent: "space-between" }}>
                    <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ color: "rgba(232,221,200,0.45)", fontSize: "0.68rem" }}>{event.date}</div>
                    </div>
                    <div style={{ display: "flex", flexShrink: 0, gap: "0.3rem" }}>
