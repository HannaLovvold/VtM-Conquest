...
function World({ mapRef, projection, terrainEnabled, onInitialIdle }) {
  ...
  // Scenario-declared initial camera (world.json `mapView`). The world payload
  // arrives after first render, so the seed happens in an effect below: the first
  // time a valid mapView shows up, the camera jumps there before the player has
  // flown anywhere, and never again.
  const { mapView } = useWorldState();
  const cameraSeededRef = useRef(false);
  useEffect(() => {
    if (!mapView || cameraSeededRef.current) return;
    const longitude = Number(mapView.longitude);
    const latitude = Number(mapView.latitude);
    if (!Number.isFinite(longitude) || !Number.isFinite(latitude)) return;
    cameraSeededRef.current = true;
    const zoom = Number.isFinite(Number(mapView.zoom))
      ? Math.min(16, Math.max(2.25, Number(mapView.zoom)))
      : viewStateRef.current.zoom;
    viewStateRef.current = { ...viewStateRef.current, longitude, latitude, zoom };
    const map = mapRef?.current?.getMap?.();
    if (map) map.jumpTo({ center: [longitude, latitude], zoom });
  }, [mapView, mapRef]);
