...
export function useWorldState() {
  ...
  const derived = {
    ...
    // Scenario-declared initial map camera {longitude, latitude, zoom} from
    // world.json (World.jsx seeds the camera from it once, on first arrival).
    mapView: state?.mapView && typeof state.mapView === "object" ? state.mapView : null,
  ...
  const output =
    ...
    JSON.stringify(prev.mapView) === JSON.stringify(derived.mapView) &&
