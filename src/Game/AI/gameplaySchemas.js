...
const unitSchema = {
  ...
  properties: {
    ...
    type: {
      ...
      enum: ["coterie", "hounds", "proxies", "retainers", "sorcery", "havenguard"],
    },
...
const worldStateOpSchema = {
  type: "object",
  description:
    "A movement of the city's global tracks. Values are DELTAS: negative lowers "
    + "Masquerade Integrity, positive raises it; positive raises Inquisition Heat, "
    + "negative cools it. Include an entry whenever this night's events strain or "
    + "soothe the Masquerade or draw mortal attention.",
  properties: {
    masqueradeIntegrity: {
      type: "number",
      description:
        "Change to Masquerade Integrity (0-100 scale, currently tracked globally). "
        + "Use a NEGATIVE number for breaches, exposures, surveillance footage, "
        + "witnesses who talked; positive for successful cover-ups and repaired secrecy.",
    },
    inquisitionHeat: {
      type: "number",
      description:
        "Change to Inquisition Heat (0-100 scale). POSITIVE when mortals — SI "
        + "agents, police, journalists, hunters — get closer to the Kindred; "
        + "negative when trails go cold.",
    },
    reason: textSchema("Brief reason the tracks moved."),
  },
  additionalProperties: false,
};
...
const impactsSchema = {
  ...
  properties: {
    ...
    worldStateOps: {
      type: "array",
      description:
        "Movements of the city's global tracks — Masquerade Integrity and "
        + "Inquisition Heat. REQUIRED whenever the event text describes a Masquerade "
        + "breach, a cover-up, mortal attention, or an Inquisition lead, or the HUD "
        + "will not match the story.",
      items: worldStateOpSchema,
    },
    markerOps: {
      type: "array",
      description:
