...
export const buildWorldSummary = async (bundle, regionCatalog = null) => {
  ...
  return [
    ...
    // Global VtM tracks: current values the AI moves via impacts.worldStateOps
    // (see ACTIONS_REFERENCE). Stated every prompt so deltas stay proportionate.
    `Masquerade Integrity: ${world.masqueradeIntegrity}/100 (100 = the veil is perfect;`
      + ` below 50 the World of Darkness is showing, below 20 the city is one breach from panic)`,
    `Inquisition Heat: ${world.inquisitionHeat}/100 (0 = mortals suspect nothing; above 60 the`
      + ` Second Inquisition runs active operations in the city, above 80 it is hunting)`,
